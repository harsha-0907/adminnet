...
pipInstallation() {
	...
	for package in ${packages[@]}; do
		command=" echo '$password' | sudo -S pip install $package ;"
		commands+=("$command")
	done	
}
...
aptInstallation() {
	...
	for package in ${packages[@]}; do
		echo "$package"
		command="echo '$password' | sudo -S apt -y install $package"
		commands+=("$command")
	done
}
...
aptGetInstallation() {
	...
	for package in ${packages[@]}; do
		echo "$package"
		command="echo '$password' | sudo -S apt-get -y install $package"
		commands+=("$command")
	done
}
...
dpkgInstallation() {
	...
	for path in ${paths[@]}; do
		command="echo '$password' | sudo -S dpkg -i $path"
		commands+=("$commands")
	done
}
...
installation() {
	...
	if [[ $flag -eq 0 ]]; then
		commands+=("echo '$password' | sudo -S apt -y autoremove")
		commands+=("echo '$password' | sudo -S apt -y autoclean")
	fi
}
...
deletion() {
	...
	IFS=' ,'
	for package in ${packages[@]}; do
		command="echo '$password' | sudo -S apt -y purge $package"
		commands+=("$command")
	done
	commands+=("echo '$password' | sudo -S apt -y autoremove")
	commands+=("echo '$password' | sudo -S apt -y autoclean")
...
exitSession() {
	...
	case $choice in
		's'|'S'|'1')
			commands+=("echo '$password' | sudo -S shutdown now")
			echo "Shutdown Added..."
			;;
		'r'|'R'|'2')
			commands+=("echo '$password' | sudo -S reboot")
			echo "Shutdown Added..."
			;;
		*)
			;;
